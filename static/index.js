...
const recordKeys = (elem, timeLimit) => {
  ...
  const keydownHandler = (e) => {
    ...
    savePush.push(e.key);
    elem.placeholder = "Selected Key(s): " + savePush.join(" + ");
    document.querySelectorAll(".panic").forEach((e) => {
      e.placeholder = "Selected Key(s): " + savePush.join(" + ");
    });
  };
  ...
  if (activeTab) {
    ...
    if (/^(abyss:)(\/)*[a-z]*/.test(url)) {
      ...
      if (override) {
        ...
        switch (chosenBackend) {
          case "uv":
            iframe.src = "/classes/uv/" + __uv$config.encodeUrl(url);
            break;
          case "dynamic":
            iframe.src = "/classes/dyn/route?url=" + encodeURIComponent(url);
            break;
          default:
...
function removeBookmark(bookmark) {
  ...
  if (bookmarks.length <= 0) {
    document.getElementById("tabContainer").style.height = "calc(100% - 168px)";
    document.getElementById("headerArea").style.height = "158px";
  }
